...
class WebRtcService {
  dynamic get _bridge =>
      js_util.getProperty(js_util.globalThis, 'WebRtcBridge');
  ...
  Future<void> initialize({
    ...
    await _promise(
      js_util.callMethod(_bridge, 'initialize', [portal, channel]),
    );
  }

  Future<String> createOffer() async =>
      (await _promise(js_util.callMethod(_bridge, 'createOffer', const [])))
          as String;
  Future<String> acceptOffer(String offer) async =>
      (await _promise(js_util.callMethod(_bridge, 'acceptOffer', [offer])))
          as String;
  Future<String> applyAnswer(String answer) async => (await _promise(
    js_util.callMethod(_bridge, 'applyAnswer', [answer]),
  )).toString();
  ...
  int connectionCount() =>
      (js_util.callMethod(_bridge, 'connectionCount', const []) as num).toInt();
  ...
  List<String> drainMessages() {
    final raw =
        js_util.callMethod(_bridge, 'drainMessages', const []) as String;
